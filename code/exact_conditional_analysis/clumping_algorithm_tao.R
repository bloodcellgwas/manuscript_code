...
## TAO WANTS TO CLUMP WITH P VALUE THRESHOLD 0.1 (1)
## THEN HE WANTS TO ASSIGN THE MINIMUM P VALUE ACROSS ALL TRAITS TO EACH VARIANT (2)
## HE WILL USE THE P VALUE TO IDENTIFY THE SENTINAL VARIANT PER CLUMP
## THIS SCRIPT PERFORMS STEP (1)
...
plink.ld <- plink.ld[plink.ld$R2 > 0.1,]

## loop through all the vars
##  if var is in LD > 0.1 with any previous clump, merge into that clump
##  if var is in LD > 0.1 with more than one previous clump, merge both
...
get_partners <- function(var) {
  partnersA <- plink.ld[plink.ld$hg19A == var & plink.ld$R2 > 0.1,]$hg19B
  partnersB <- plink.ld[plink.ld$hg19B == var & plink.ld$R2 > 0.1,]$hg19A
...
write.table(hg19_table, paste(OUT_DIR, "/condout/ldclump_dosage/hg19_clump_table_tao.tsv", sep=""), sep="\t", quote=FALSE, row.names=F)
