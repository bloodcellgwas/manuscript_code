...
print(args.chrom)
hd5_variants = pd.read_csv(OUT_DIR+"/condout/subgenhd5/ids_pos_"+str(args.chrom)+".tsv", header=None)[0]
chr_data = h5py.File(OUT_DIR+"/condout/subgenhd5/chr"+str(args.chrom)+"_pos_clean_id.h5", "r")
...
variants_select = np.where(hd5_variants.isin(chrom_condsig))[0]
...
corrdf.to_csv(OUT_DIR+"/condout/ldclump_dosage/dosage_"+str(args.chrom)+".ld", index=False)
